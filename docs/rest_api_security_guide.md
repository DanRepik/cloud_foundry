...
In the prior installment of this series the Cloud foundry `rest_api` was used to implement a greet API offering a simple 'Hello world' service.  There the focus was to illustrate the basics of exposing a `python_function` as a service in a Rest API.

With the 'hello World' service we could safely neglect security leaving the API services publically available on the Internet.  In real world applications openly available services like that are the exception rather than the norm.  Typically access to the API services are restricted to authorized users. In this installment we'll explore adding security to API services.

Request authorization for API gateways is token based, where the client includes a bearer token in the authorization header in the request.  The Gateway API then validates the supplied token usually with a Lambda function.  Usually the token is a JWT token and the request is authorized if the token can be successfully decoded.  Additonally the validation function can pass the unencoded token contents to the service function allowing the function to enforce any additional fine grained access control if needed.

The process of securing an API a service, or more specifically a path operation, involves two steps:
...
The `rest_api` function allows you to configure token validators that the gateway will use to authenticate and authorize API requests. Token validators can be implemented using either AWS Cognito User Pools or custom Lambda functions.

### Understanding the `token_validators` Argument

The `token_validators` argument in the `rest_api` function defines how incoming API requests are authenticated and authorized. Each token validator links a security schema name to a specific validation mechanism, such as an AWS Cognito User Pool or a custom Lambda function.
...
**Example: Using a Cognito User Pool**
...
token_validators = [
    {
        ...
        "function": custom_auth_lambda_function,
    }
]
```
...
paths:
    /secure-greet:
        get:
            ...
            security:
                - cognito_validator: []  # Matches the token validator name
            responses:
                '200':
                    description: Successful response
```

> Note that in the Open API specification security is an array allowing multiple security schemes to be assigned path operations.  In this case a request is authorized if at least one of the security schemes is satisfied.
