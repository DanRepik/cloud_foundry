...
class DocumentBucket(pulumi.ComponentResource):
  def __init__(self, name, bucket_name: str = None, notifications=None, opts=None):
    ...
    # Create an IAM role for the Lambda function
    self.lambda_role = iam.Role(f"{name}-lambda-role",
      assume_role_policy=json.dumps(assume_role_policy),
      opts=pulumi.ResourceOptions(parent=self)
    )

    # Attach the necessary policies to the role
    iam.RolePolicyAttachment(f"{name}-lambda-policy-attachment",
  ...
  def add_notification(self, notification: dict):
    ...
    bucket_notification = s3.BucketNotification(f"{self.bucket._name}-notification",
    ...
    lambda_permission = lambda_.Permission(f"{lambda_function._name}-permission",
